...
import timeRange from 'time-span';
...
test('async with concurrency: 2 (out of time sequence)', async t => {
...
test('aggregate errors when stopOnError is false', async t => {
  ...
  await t.throwAsync(pMap(errorInput1, mapper, {concurrency: 1, stopOnError: false}), {instanceOf: AggregateError, message: /foo(.|\n)*bar/});
  await t.throwsAsync(pMap(errorInput2, mapper, {concurrency: 1, stopOnError: false}), {instanceOf: AggregateError, message: /bar(.|n)*foo/});
});
```
